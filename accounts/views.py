...
def user_details(request): 
    ...
    # Attempt to get FitFinder details 
    try: 
        fitFinder = FitFinder.objects.get(user=request.user) 
        data['fitFinder'] = fitFinder.id 
    except FitFinder.DoesNotExist: 
        data['fitFinder'] = None 
        
    
    # Attempt to get FitMaker details 
    try: 
        fitMaker = FitMaker.objects.get(user=request.user) 
        data['fitMaker'] = fitMaker.id
...
def all_users_details(request):
    ...
    for user in users:
        ...
        # Check if the user is a FitFinder
        try:
            fitFinder = FitFinder.objects.get(user=user)
            data['fitFinder'] = fitFinder.id
        except FitFinder.DoesNotExist:
            data['fitFinder'] = None

        # Check if the user is a FitMaker
        try:
            fitMaker = FitMaker.objects.get(user=user)
            data['fitMaker'] = fitMaker.id
        except FitMaker.DoesNotExist:
            data['fitMaker'] = None
